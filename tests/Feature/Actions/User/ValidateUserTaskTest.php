...
describe(CompleteUserTask::class, function () {
    it('can validate task', function () {
        ...
        $this->freezeTime(function (Carbon $time) use ($user, $task) {
            app(CompleteUserTask::class)->handle($user->id, $task->id);
    ...
    it('should not validate task if user is not the one assigned', function () {
        ...
        app(AssignUserTask::class)->handle($task->id, $user1->id);
        app(CompleteUserTask::class)->handle($user2->id, $task->id);
    ...
    it('cannot validate task already completed', function () {
        ...
        $this->assertThrows(fn () => app(CompleteUserTask::class)->handle($user->id, $task->id), CannotValideAlreadyCompletedTaskException::class);
    });
});
